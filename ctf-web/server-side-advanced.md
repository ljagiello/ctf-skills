...
See also: [server-side-advanced-2.md](server-side-advanced-2.md) for Part 2 (SSRF-to-Docker API RCE chain, Castor XML xsi:type deserialization, Apache ErrorDocument expression file read, SQLite file path traversal, HQL injection via non-breaking space, base64-encoded path traversal, Windows 8.3 short filename bypass, URL parse_url @ symbol bypass).
